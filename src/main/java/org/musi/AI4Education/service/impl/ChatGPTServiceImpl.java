...
public class ChatGPTServiceImpl extends ServiceImpl<ChatHistoryMapper, ChatHistory> implements ChatGPTService {
    ...
    public String getChatWav(String question,String chatHistory) {
        ...
        try {
            ProcessBuilder pb;
            if (question.equals("")) {
                pb = new ProcessBuilder("G:\\connectChatGPT\\venv\\Scripts\\python.exe", "G:\\green-farm\\src\\main\\java\\Python_API\\Wenxin+GPT4\\main.py");
            } else {
                Gson gson = new Gson();
                String jsonInput = gson.toJson(chatHistory);
                pb = new ProcessBuilder("G:\\connectChatGPT\\venv\\Scripts\\python.exe", "G:\\green-farm\\src\\main\\java\\Python_API\\Wenxin+GPT4\\main.py",question,jsonInput);
            }
            Process p = pb.start();
    ...
    public List<HashMap<String,String>> connectWithChatGPT(String question, String qid) throws JSONException {
        ...
        //获取大模型输出的答案
        String answer = getChatWav(question,chatHistoryTemp);
